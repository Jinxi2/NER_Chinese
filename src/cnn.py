...
class CNN():
    def __init__(self, args, embeddings, tag2label, vocab, paths, config):
        ...
        self.config = config
        self.filter_sizes = [2,3]  # size of filter
        self.filter_num = 3
    ...
    def CNN_layer_op(self):
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.name_scope("Conv-maxpool-%s" % filter_size):
                # convlution layer
                filter_shape = [filter_size, len(self.embeddings), 1, self.filter_num]
                ...
                pool = tf.nn.max_pool(h,ksize=[1,self.sequence_lengths-filter_size+1,1,1],strides=[1,1,1,1],padding='VALID',name='maxpool')
        ...
        #
        # with tf.variable_scope("proj"): # 投影
    ...
    def loss_op(self):
        with tf.name_scope('loss'):
            loss = tf.nn.softmax_cross_entropy_with_logits(self.scores,self.labels)
            self.loss = tf.reduce_mean(loss)
        # 输出误差
            tf.summary.scalar("loss",self.loss)
        with tf.name_scope('accuracy'):
            crt = tf.equal(self.predctions, tf.argmax(self.labels, 1))
            self.acc = tf.reduce_mean(tf.cast(crt,'float'), name='accuracy')
    ...
    def softmax_pred_op(self):
        ...
        with tf.name_scope('output'):
            W = tf.Variable(tf.truncated_normal([self.filter_num * len(self.filter_sizes), 7], stddev=0.1), name="W")
            b = tf.Variable(tf.constant(0.1, shape=[7],name='b'))
            self.scores = tf.nn.xw_plus_b(self.h_drop, W, b, name='scores')
            self.predctions = tf.argmax(self.scores, 1, name='predictions')
    ...
    def run_one_epoch(self, sess, train, dev, tag2label, epoch, saver):
        ...
        for step, (seqs, labels) in enumerate(10):
